# Qwen Attempt 3: Fails because it computes paths through u but uses the same r for all children, not distributing across pairs.
import sys
...
def dfs(u, parent, adj, max_diam, max_down, k):
    ...
    for r in range(k + 1):
        ...
        # Incorrect: Collects vals for r but doesn't handle s1 + s2 = r
        vals = []
        for v, w in children:
            if max_down[v][r] != float('-inf'):
                vals.append(w + max_down[v][r])
        if len(vals) >= 2:
